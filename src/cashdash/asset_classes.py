...
class Debt(AssetClass):
    ...
    def calc_agg_data(self) -> None:
        ...
        # Separate balance changes to user spending and interest accrueing
        df["Spent"] = np.where(df["ADDED_DEBT"]==False, df["AMOUNT"], 0)
        df["Interest"] = np.where(df["INTEREST_FLAG"]==True, df["AMOUNT"], 0)
        df.drop(columns=["ADDED_DEBT", "INTEREST_FLAG"], inplace=True)
        
        # Create current worth column
        df[["Spent", "Interest", "Current worth"]] = df[["Spent", "Interest", "AMOUNT"]].cumsum()
